...
describe('store.ts', () => {
    ...
    describe('readPluginsConfig', () => {
        ...
        test('should read and parse YAML config', async () => {
  ...
  - id: test-plugin
    ...
    config:
      key: value`
        ...
        test('should filter out invalid plugins', async () => {
            const yamlContent = `plugins:
  - id: valid-plugin
    module: ./valid.js
  - id: ''
  ...
  - id: no-module
    module: ''`
        ...
        test('should set enabled=true by default', async () => {
            const yamlContent = `plugins:
  - id: test-plugin
    module: ./test.js`
  ...
  - id: disabled-plugin
    ...
    describe('normalizeModuleSpecifierForPluginsConfig', () => {
        test('should handle module spec without prefix', async () => {
            vi.mocked(fs.realpath).mockImplementation(async (p) => String(p))
        ...
        test('should fall back when realpath fails', async () => {
        ...
        test('should normalize relative path', async () => {
            vi.mocked(fs.realpath).mockImplementation(async (p) => String(p))
        ...
        test('should throw error for paths outside DATA_DIR', async () => {
        ...
        test('should throw error for empty module', async () => {
            await expect(
                store.normalizeModuleSpecifierForPluginsConfig('')
            ).rejects.toThrow('Missing module')
        })

        test('should handle absolute paths within DATA_DIR', async () => {
            vi.mocked(fs.realpath).mockImplementation(async (p) => String(p))
        ...
        test('should keep absolute path outside plugins dir but inside DATA_DIR', async () => {
            vi.mocked(fs.realpath).mockImplementation(async (p) => String(p))
        ...
        test('should keep absolute path when resolving to DATA_DIR root', async () => {
            vi.mocked(fs.realpath).mockImplementation(async (p) => String(p))
    ...
    describe('upsertPluginConfig', () => {
        test('should add new plugin to empty config', async () => {
        ...
        test('should update existing plugin', async () => {
  ...
  - id: existing-plugin
    ...
    enabled: false`
        ...
        test('should infer ID from module path if not provided', async () => {
            vi.mocked(fs.access).mockRejectedValue(new Error('Not found'))
            vi.mocked(fs.realpath).mockImplementation(async (p) => String(p))
            vi.mocked(fs.mkdir).mockResolvedValue(undefined)
            vi.mocked(fs.writeFile).mockResolvedValue(undefined)
        ...
        test('should infer ID from non-index module path', async () => {
            vi.mocked(fs.access).mockRejectedValue(new Error('Not found'))
            vi.mocked(fs.realpath).mockImplementation(async (p) => String(p))
            vi.mocked(fs.mkdir).mockResolvedValue(undefined)
            vi.mocked(fs.writeFile).mockResolvedValue(undefined)
        ...
        test('should sanitize plugin ID', async () => {
            vi.mocked(fs.access).mockRejectedValue(new Error('Not found'))
            vi.mocked(fs.realpath).mockImplementation(async (p) => String(p))
            vi.mocked(fs.mkdir).mockResolvedValue(undefined)
            vi.mocked(fs.writeFile).mockResolvedValue(undefined)
        ...
        test('should sort plugins alphabetically', async () => {
            const existingConfig = `plugins:
  - id: zebra-plugin
    module: ./zebra.js`
        ...
        test('should preserve config and source fields', async () => {
            vi.mocked(fs.access).mockRejectedValue(new Error('Not found'))
            vi.mocked(fs.realpath).mockImplementation(async (p) => String(p))
            vi.mocked(fs.mkdir).mockResolvedValue(undefined)
    ...
    describe('patchPluginConfig', () => {
        test('should patch existing plugin', async () => {
            const existingConfig = `plugins:
  - id: test-plugin
    module: ./test.js
    enabled: true`
        ...
        test('should create plugin if not found', async () => {
            vi.mocked(fs.access).mockRejectedValue(new Error('Not found'))
            vi.mocked(fs.realpath).mockImplementation(async (p) => String(p))
            vi.mocked(fs.mkdir).mockResolvedValue(undefined)
            vi.mocked(fs.writeFile).mockResolvedValue(undefined)
        ...
        test('should update module path', async () => {
            const existingConfig = `plugins:
  - id: test-plugin
    module: ./old.js`
        ...
        test('should update source when provided', async () => {
            const existingConfig = `plugins:
  - id: test-plugin
    module: ./test.js
    enabled: true`
        ...
        test('should create plugin when missing with explicit module', async () => {
            vi.mocked(fs.access).mockRejectedValue(new Error('Not found'))
            vi.mocked(fs.realpath).mockImplementation(async (p) => String(p))
            vi.mocked(fs.mkdir).mockResolvedValue(undefined)
            vi.mocked(fs.writeFile).mockResolvedValue(undefined)
    ...
    describe('removePluginConfig', () => {
        test('should remove existing plugin', async () => {
            const existingConfig = `plugins:
  - id: plugin1
    module: ./plugin1.js
  - id: plugin2
  ...
  - id: test-plugin
    ...
    describe('Edge Cases', () => {
        ...
        test('should handle very long plugin IDs', async () => {
            vi.mocked(fs.access).mockRejectedValue(new Error('Not found'))
            vi.mocked(fs.realpath).mockImplementation(async (p) => String(p))
            vi.mocked(fs.mkdir).mockResolvedValue(undefined)
            vi.mocked(fs.writeFile).mockResolvedValue(undefined)
        ...
        test('should handle special characters in module paths', async () => {
            vi.mocked(fs.realpath).mockImplementation(async (p) => String(p))
    ...
    describe('internal helpers', () => {
        test('resolveDataDir should use process.env when env is empty', async () => {
        ...
        test('resolveDataDir should fall back to default when env and process missing', async () => {
            const envModule = await import('../../../domain/models/env')
        ...
        test('parseConfig should ignore non-string fields', () => {
        ...
        test('parseConfig should default to empty plugins when plugins is not an array', () => {
            const parsed = store.__testing.parseConfig(JSON.stringify({
        ...
        test('parseConfig should use JSON parser for non-YAML extension', () => {
        ...
        test('inferIdFromModule should handle file URLs', () => {
            const result = store.__testing.inferIdFromModule('file:///test/data/plugins/index.js')

            expect(result).toBe('plugins')
        })

        test('inferIdFromModule should fall back when parent is missing', () => {
            const result = store.__testing.inferIdFromModule('/index.js')

            expect(result).toBe('plugin')
        })

        test('inferIdFromModule should fall back when base is empty', () => {
            const result = store.__testing.inferIdFromModule('')

            expect(result).toBe('plugin')
        })

        test('sanitizeId should fall back for empty values', () => {
